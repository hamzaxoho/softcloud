...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findByprojects/:key", authorize([]), UserController.findByprojects);
  ...
  */
  findByprojects: async (req, res) => {
    try {
      const result = await UserModel.findByprojects(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
