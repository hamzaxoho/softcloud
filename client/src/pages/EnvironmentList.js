...
class EnvironmentList extends Component {
  ...
  confirmDialogDelete(id) {
    this.props.actionsEnvironment.deleteEnvironment(this.state.idDelete).then(data => {
      this.props.actionsEnvironment.loadEnvironmentList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/environments/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.EnvironmentListReducer.listEnvironment
  };
}
