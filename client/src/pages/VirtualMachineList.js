...
class VirtualMachineList extends Component {
  ...
  // Load data on start
  componentWillMount() {
    this.props.actionsVirtualMachine.loadVirtualMachineList();
  }
  ...
  confirmDialogDelete(id) {
    this.props.actionsVirtualMachine.deleteVirtualMachine(this.state.idDelete).then(data => {
      this.props.actionsVirtualMachine.loadVirtualMachineList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
  ...
  render() {
    ...
    const link = "/virtualmachines/";

    return (
      <div>
        <h1>VirtualMachine List</h1>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right">DiskSize</TableCell>
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/virtualmachines/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
                <TableCell align="right">{ row.CPUS }</TableCell>
                <TableCell align="right">{ row.DiskSize }</TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.VirtualMachineListReducer.listVirtualMachine
  };
}
